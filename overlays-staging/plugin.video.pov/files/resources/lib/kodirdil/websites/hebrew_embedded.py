...
def get_local_embedded_db_path():
    ...
    profile_path = kodi_utils.translate_path('special://profile/addon_data/plugin.video.pov/')
...
def init_local_embedded_db():
    ...
    try:
        ...
        kodi_utils.logger("POV-HEBSUBS", f"EMBEDDED | Error initializing local DB: {str(e)}")
...
def add_to_local_embedded_db(tagline, media_type):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        kodi_utils.logger("POV-HEBSUBS", f"EMBEDDED | Added to local DB: {tagline_lower[:50]}...")
        ...
        kodi_utils.logger("POV-HEBSUBS", f"EMBEDDED | Error adding to local DB: {str(e)}")
...
def get_local_embedded_taglines(media_type=None):
    ...
    try:
        ...
        kodi_utils.logger("POV-HEBSUBS", f"EMBEDDED | Loaded {len(taglines)} taglines from local DB")
        return taglines
    except Exception as e:
        kodi_utils.logger("POV-HEBSUBS", f"EMBEDDED | Error reading local DB: {str(e)}")
        return set()
...
def clear_local_embedded_db():
    ...
    try:
        ...
        if os.path.exists(db_path):
            os.remove(db_path)
            kodi_utils.logger("POV-HEBSUBS", "EMBEDDED | Cleared local learning DB")
    except Exception as e:
        kodi_utils.logger("POV-HEBSUBS", f"EMBEDDED | Error clearing local DB: {str(e)}")
    ...
    try:
        ...
        if cache_key in _taglines_cache and cache_key in _cache_timestamps:
            ...
            if cache_age_hours < CACHE_DURATION_HOURS:
                kodi_utils.logger("POV-HEBSUBS", f"EMBEDDED | Using cached taglines ({len(_taglines_cache[cache_key])} items)")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                kodi_utils.logger("POV-HEBSUBS", f"EMBEDDED | Loaded {len(remote_taglines)} remote + {len(combined_taglines) - len(remote_taglines)} local taglines")
        except Exception as e:
            kodi_utils.logger("POV-HEBSUBS", f"EMBEDDED | Remote fetch failed (using local only): {str(e)}")
        ...
        kodi_utils.logger("POV-HEBSUBS", f"EMBEDDED | Error fetching taglines: {str(e)}")
        return None
...
def clear_taglines_cache():
    ...
    _taglines_cache = {}
    _cache_timestamps = {}
    kodi_utils.logger("POV-HEBSUBS", "EMBEDDED | Cleared taglines cache")
