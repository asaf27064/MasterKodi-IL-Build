...
def is_ktuvit_enabled():
    return get_setting('hebrew_subtitles.match_ktuvit', 'true') == 'true'
...
def search_for_subtitles(media_metadata):
    ...
    if not is_ktuvit_enabled():
        kodi_utils.logger("POV-HEBSUBS", "Ktuvit search disabled. Skipping...")
        return []
    ...
    kodi_utils.logger("POV-HEBSUBS", f"[KTUVIT] Searching: {media_type} - {title} S{season}E{episode}")
    ...
    try:
        ...
        kodi_utils.logger("POV-HEBSUBS", f"[KTUVIT] Error: {str(e)}")
        return []
