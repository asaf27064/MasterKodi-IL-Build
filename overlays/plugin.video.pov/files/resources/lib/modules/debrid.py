		...
		try:
			...
			for i in files or []:
				...
				if season:
					...
					########### KODIRDIL - extras filter for EPISODES too ###########
					# Upstream runs this only in the `elif` below, i.e. for MOVIES.
					# An episode was filtered by seas_ep_filter alone -- and a file
					# named 'silo.s03e08...-cakes-sample.mkv' matches s03e08 just as
					# well as the real one, so the SAMPLE was a valid candidate
					# (Asaf 2026-08-22: 182MB sample played instead of the 9GB
					# episode; container Duration 65.0s = the 1:05 he saw).
			...
			########### KODIRDIL - biggest match wins, episodes included ###########
			# Upstream sorted by size only `if not season`, so for an episode the
			# picker took the FIRST entry in the pack's arbitrary order. Sorting
			# always is the second, independent guard against the same bug: even
			# if an extra slips past the filter, the full episode outweighs it.
