...
def get_minimum_sync_percent():
    return int(get_setting('hebrew_subtitles.minimum_sync_percent', '70'))
...
def search_hebrew_subtitles_on_website(website_info, media_metadata, website_subtitles_dict, lock):
    ...
    try:
        ...
        kodi_utils.logger("POV-HEBSUBS", f"{website_info['short_name']}_subtitles_list: {str(hebrew_subtitles_list)}")

    except Exception as e:
        kodi_utils.logger("POV-HEBSUBS", f"Error in searching Hebrew subtitles from {website_info['website']}: {str(e)}")
...
def search_hebrew_subtitles_for_selected_media(media_type, title, season, episode, year, tmdb_id, imdb_id):
    ...
    if current_media_metadata_in_cache and current_media_metadata_in_cache == input_params:
        kodi_utils.logger("POV-HEBSUBS", "current_media_metadata_in_cache is the same. Skipping subtitles search...")
    ...
    kodi_utils.logger("POV-HEBSUBS", f"Starting Hebrew subtitles search for: {media_metadata}")
    ...
    kodi_utils.logger("POV-HEBSUBS", f"unique_subtitles_list: {str(unique_subtitles_list)}")
...
def generate_subtitles_match_top_panel_text_for_sync_percent_match(
    ...
    kodi_utils.logger("POV-HEBSUBS", f"Sources with matched subtitles: {total_subtitles_matches_count}")
...
def calculate_highest_sync_percent_and_set_match_text(
    ...
    if search_embedded and hebrew_embedded_taglines:
        ...
        if is_hebrew_embedded_tagline_match_found:
            ...
            kodi_utils.logger("POV-HEBSUBS", f"EMBEDDED | Match found! For: {original_video_tagline}")
    ...
    if total_subtitles_found_list:
        ...
        if highest_sync_percent >= minimum_sync_percent:
            ...
            kodi_utils.logger("POV-HEBSUBS", f"Match found! {highest_sync_percent}% | Source: {original_video_tagline} | Subtitle: {matched_subtitle_name}")
