...
def get_release_quality(release_info):
	...
	if any(i in release_info for i in ('.tc.', '.ts.', 'tsrip', 'hdts', 'hdtc', '.hd.tc', 'dvdts', 'telesync', 'tele.sync', 'telecine', 'tele.cine')):
		return 'TELE'
	########### KODIRDIL - canonical resolution list, highest tier FIRST ###########
	# Upstream tested 720 -> 1080 -> 4K, so a name carrying more than one marker
	# was labelled by the LOWEST tier it happened to mention. Added the same
	# tokens POV now knows: 8K/4320p, 1440p/2K/QHD and the WIDTHxHEIGHT forms.
	# 4320p IS 8K and 1440p/2K is not 1080p, but POV and Gears only have four
	# buckets -- 4K / 1080p / 720p / SD -- so each folds into the nearest one.
	# A new label would sort last and drop out of the source-count panel.
	if any(i in release_info for i in ('4320p', '.8k', '.4k', 'hd4k', '4khd', '.uhd', 'ultrahd', 'ultra.hd', 'hd2160', '2160hd', '2160', '2160p', '216o', '216op', '3840x2160', '4096x2160')):
		return '4K'
	if any(i in release_info for i in ('1080', '1080p', '1080i', 'hd1080', '1080hd', 'hd1080p', 'm1080p', 'fullhd', 'full.hd', '.fhd', '1440p', '.2k', 'qhd', '1920x1080', '1o8o', '1o8op', '108o', '108op', '1o80', '1o80p')):
		return '1080p'
	if any(i in release_info for i in ('720', '720p', '720i', 'hd720', '720hd', 'hd720p', '72o', '72op', '1280x720')):
		return '720p'
	################################################################
	return None
