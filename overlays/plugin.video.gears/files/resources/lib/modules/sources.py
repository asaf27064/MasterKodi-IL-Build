...
class Sources():
	...
	def get_sources(self):
		...
		if enable_hebrew_subtitles:
			try:
				kodi_utils.logger("Gears-HEBSUBS", "Starting Hebrew subtitles search thread...")
				media_type = self.media_type
				title = self.meta.get('title', '')
				season = str(self.meta.get('season', ''))
				episode = str(self.meta.get('episode', ''))
				year = str(self.meta.get('year', ''))
				tmdb_id = str(self.meta.get('tmdb_id', ''))
				imdb_id = str(self.meta.get('imdb_id', ''))

				search_hebrew_subtitles_thread = thread_utils.create_search_hebrew_subtitles_thread(
					media_type, title, season, episode, year, tmdb_id, imdb_id
				)
