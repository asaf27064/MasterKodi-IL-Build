...
class Movies:
	...
	def build_movie_content(self, _position, _id):
		try:
			...
			# Gears's stock code only fetches OMDB ratings when the Extras window opens.
			# To show IMDb / Metacritic / RT / TMDb ratings in the list view itself, pre-fetch here.
			...
			# Skin code (Estuary Hebrew + Arctic Fuse Hebrew) reads ListItem.Property(gears.<key>_rating)
			# and the matching _icon for IMDb / Metacritic / RT / TMDb dots in the listitem.
			rating_props = {}
	...
	def worker(self):
		...
		# Only enable if user hasn't disabled extra ratings via settings.
