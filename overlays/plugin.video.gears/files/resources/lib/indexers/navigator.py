...
class Navigator:
	...
	def genres(self):
		...
		########### KODIRDIL - Use per-genre icon (genre_action.png etc.) instead of one 'genres' icon ###########
		# Gears stock collapses every genre to the same 'genres' icon. We assign a distinct
		# icon per genre via meta_lists.py's 'icon' key. Falls back to 'genres' if the icon key is
		# missing, so the patch is safe if meta_lists.py ever loses the 'icon' fields.
