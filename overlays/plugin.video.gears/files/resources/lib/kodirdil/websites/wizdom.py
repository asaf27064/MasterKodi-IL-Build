...
def is_wizdom_enabled():
    return get_setting('gears.hebrew_subtitles.match_wizdom', 'true') == 'true'
...
def search_for_subtitles(media_metadata):
    ...
    if not is_wizdom_enabled():
        kodi_utils.logger("Gears-HEBSUBS", "Wizdom search disabled. Skipping...")
        return []
    ...
    kodi_utils.logger("Gears-HEBSUBS", f"[WIZDOM] Searching: {media_type} - {title} S{season}E{episode} imdb:{imdb_id}")
    ...
    try:
        ...
        kodi_utils.logger("Gears-HEBSUBS", f"[WIZDOM] Error: {str(e)}")
        return []
    ...
    kodi_utils.logger("Gears-HEBSUBS", f"[WIZDOM] Found {len(wizdom_subtitles_list)} subtitles")
