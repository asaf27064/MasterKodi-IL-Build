...
def get_meta_language():
	"""Get the configured metadata language (default: en for English).
	When set to 'he', TMDb returns Hebrew overviews/translations for media titled in Hebrew markets."""
	return get_setting('gears.meta_language', 'en') or 'en'

def get_include_image_language():
	"""Image language hint for TMDb. We always include 'en' and 'null' as fallbacks so we never
	end up with no image. Hebrew movie posters are rare; this keeps the artwork rich."""
...
def movie_details(tmdb_id, api_key):
	try:
		lang = get_meta_language()
		img_lang = get_include_image_language()
		url = 'https://api.themoviedb.org/3/movie/%s?api_key=%s&language=%s&append_to_response=external_ids,videos,credits,release_dates,alternative_titles,translations,' \
		'images,keywords&include_image_language=%s' % (tmdb_id, api_key, lang, img_lang)
...
def tvshow_details(tmdb_id, api_key):
	try:
		lang = get_meta_language()
		img_lang = get_include_image_language()
		url = 'https://api.themoviedb.org/3/tv/%s?api_key=%s&language=%s&append_to_response=external_ids,videos,credits,content_ratings,alternative_titles,translations,' \
		'images,keywords&include_image_language=%s' % (tmdb_id, api_key, lang, img_lang)
		return get_tmdb(url).json()
	except: return None
...
def movie_set_details(collection_id, api_key):
	try:
		# KODIRDIL — use configured meta language so collection (movie set) synopses match he/en preference
		lang = get_meta_language()
		url = 'https://api.themoviedb.org/3/collection/%s?api_key=%s&language=%s' % (collection_id, api_key, lang)
		return get_tmdb(url).json()
	except: return None
...
def season_episodes_details(tmdb_id, season_no):
	...
	if api_key in (None, 'empty_setting', ''): return no_api_key()
	try:
		# KODIRDIL — use configured meta language so per-EPISODE overviews come back in Hebrew when set.
		# This endpoint returns the whole season including each episode's overview, so without lang=he
		# the user sees the show synopsis in Hebrew (from tvshow_details) but every episode in English.
		lang = get_meta_language()
		url = 'https://api.themoviedb.org/3/tv/%s/season/%s?api_key=%s&language=%s&append_to_response=credits' % (tmdb_id, season_no, api_key, lang)
		return get_tmdb(url).json()
	except: return None
