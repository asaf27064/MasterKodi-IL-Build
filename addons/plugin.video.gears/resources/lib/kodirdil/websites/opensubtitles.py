...
def is_opensubtitles_enabled():
    return get_setting('gears.hebrew_subtitles.match_opensubtitles', 'true') == 'true'
...
def _api_key():
    ...
    return get_setting('gears.hebrew_subtitles.opensubtitles_apikey', '') or random.choice(BAKED_KEYS)
...
def search_for_subtitles(media_metadata):
    ...
    if not is_opensubtitles_enabled():
        kodi_utils.logger("Gears-HEBSUBS", "OpenSubtitles search disabled. Skipping...")
        return []
    ...
    kodi_utils.logger("Gears-HEBSUBS", f"[OPENSUBTITLES] Searching: {media_type} - {title} S{season}E{episode}")
    ...
    try:
        ...
        if response.status_code != 200:
            kodi_utils.logger("Gears-HEBSUBS", f"[OPENSUBTITLES] HTTP {response.status_code}")
            return []
        ...
        kodi_utils.logger("Gears-HEBSUBS", f"[OPENSUBTITLES] Found {len(opensubtitles_list)} subtitles")
        return opensubtitles_list

    except Exception as e:
        kodi_utils.logger("Gears-HEBSUBS", f"[OPENSUBTITLES] Error: {str(e)}")
        return []
