...
from resources.sources import subdl_ai
...
def sort_subtitles(f_result,video_data):
    ...
    site_id_order = ['[Ktuvit]', '[Wizdom]', '[Telegram]', '[OpenSubtitles]', '[subdl]', '[YIFY]', '[SubSource]', '[Subscene]', '[BSPlayer]']
    ...
    # De-duplicate identical rows (same language + filename + site) -- e.g.
    # OpenSubtitles often returns the same release twice with different file
    # ids. Lists are already sorted best-first, so keeping the FIRST occurrence
    # keeps the higher match%.
    seen = set()
    deduped = []
...
def c_get_subtitles(video_data, all_lang_override=False):
    ...
    for source in [ktuvit, wizdom, telegram, opensubtitles, yify, subsource, subscene, bsplayer, subdl_ai]:
        source.global_var = []
    ...
    # subdl (our extra English source) -- enriches the pool DarkSubs ranks, so a
    # subdl English sub can be the best match to auto-translate. English-only.
    if Addon.getSetting('subdl')=='true':
        thread.append(Thread(subdl_ai.get_subs,video_data, all_lang_override))
        all_sources.append(('subdl_ai',subdl_ai))
...
def machine_translate_subs(input_file,output_file):
    ...
    import chardet
    # chardet can return None for empty/undetectable content -- default to
    # utf-8 instead of crashing ('encoding:'+None / decode(None)).
    ...
    # MasterKodi AI (Gemini) -- our high-quality, gender-aware engine. Falls back
    # to Google if the AI returns nothing (no keys/proxy, quota, error) so the
    # user always gets a translation.
    if Addon.getSetting("translate_p")=='3':
        _heb = None
        try:
            from resources.aisubs import ai_bridge
            _heb = ai_bridge.translate_english_text(text)
        except Exception as _e:
            log.warning('[AI] auto_translate error: %s' % _e)
        if _heb == 'DECLINED':
            # User said no to auto AI translation -> no translation at all
            # (the caller keeps the English sub as-is).
            log.warning('[AI] user declined auto translation')
            return 'DECLINED'
        if _heb and _heb.strip():
            f_sub_pre = _heb
            ai_used = True
        else:
            log.warning('[AI] auto_translate produced nothing -> Google fallback')
            f_sub_pre = google_machine_translate(text, encoding)

    # Google https://translate.google.com
    elif Addon.getSetting("translate_p")=='0':
        f_sub_pre = google_machine_translate(text, encoding)
    ...
    # Skip it when our AI engine produced the text -- it already applies the RTL
    # punctuation reorder, and doing it twice would corrupt the line order.
    if Addon.getSetting("auto_fix_sub_punctuation")=='true' and not ai_used:
    ...
    if not general.break_all:
        with open(output_file, mode="w", encoding="utf8") as f:
                 f.write(f_all)
        # Record WHERE this Hebrew came from (community pool / Gemini / Google...)
        # next to the translated file, so the subtitle window can tag the row.
        try:
            if ai_used:
                origin = getattr(general, 'ai_last_origin', '') or 'Gemini AI'
            else:
                origin = {'0': 'Google Translate', '1': 'Bing', '2': 'Yandex'}.get(
                    Addon.getSetting("translate_p"), 'תרגום מכונה')
            with open(output_file + '.origin', 'w', encoding='utf-8') as f:
                f.write(origin)
        except Exception:
            pass
...
def download_sub(source,download_data,MySubFolder,language,filename):
    ...
    log.warning(filename)
    from resources.modules import general
    # Self-heal: recreate the working folders if a cache-clean / manual delete
    ...
    if 'Hebrew' in language:
        ...
        if Addon.getSetting('telegram')=='true' and source != "telegram" and not already_translated and not declined:
            log.warning(f"[Telegram] | upload_subtitle_to_telegram | source={source} | sub_file={sub_file}")
