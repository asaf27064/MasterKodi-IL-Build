...
ai_pick_pct=0
ai_pick_site=""
# True while the user is downloading an explicitly-chosen sub from the window
# (skips the auto-AI confirmation prompt).
ai_manual=False


def ensure_cache_dirs():
    """Recreate the working folders if anything deleted them mid-session
    (cache cleaners, manual deletion). Without this, a missing Cached_subs
    made every download attempt crash -> no sub placed, no auto-translate."""
    for _d in (CachedSubFolder, TransFolder, MySubFolder):
        try:
    ...
    if (show_dp):
        ...
        while(1):
            ...
            if (show_msg=="END"):
                ...
                break
            time.sleep(0.1)
            # Idle-based timeout: a long AI translation keeps updating the
            # message/percent, so reset the timer on ANY change. The dialog
            # only dies after 120s with NO progress at all (a real hang) --
            # it used to hard-close at 120s TOTAL, vanishing mid-translation.
            _state = (show_msg, progress_msg)
