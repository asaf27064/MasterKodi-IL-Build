...
# Baked fallback OpenSubtitles.com API keys. The remote key list
# (kodi7rd.github.io/.../darksubs_opensubtitles_api.json) started returning
# non-JSON, so `.json()` threw -> cache returned None -> random.choice(None)
# CRASHED the whole OpenSubtitles thread -> zero results for every language
# (this is why OpenSubtitles never appeared while All Subs Plus, which bakes its
# own keys, did). These are the same working keys All Subs Plus ships.
DEFAULT_OS_API_KEYS = [
    {"OS_API_KEY_NAME": "baked1", "OS_API_KEY_VALUE": "9bOXkEUkqg5fHTWCrOQ6pYLBlHtRd9fM"},
    {"OS_API_KEY_NAME": "baked2", "OS_API_KEY_VALUE": "3MeVyIKDINfXJPKMTAWtuGzJDYcAYTnb"},
    {"OS_API_KEY_NAME": "baked3", "OS_API_KEY_VALUE": "yh4v2XkLaz4k341i5KW3a3yma46DafjE"},
    {"OS_API_KEY_NAME": "baked4", "OS_API_KEY_VALUE": "P83ZTw2Ec3XP6h2uXwTqOI9PWbo3cpGL"},
]


def c_get_os_api_keys():
    """Fetch the remote OS key list, but ALWAYS fall back to the baked keys on
    any failure (network / non-JSON / empty) so OpenSubtitles never dies for lack
    of a key. Baked keys are appended as extra fallbacks even on success."""
    try:
        remote = requests.get('https://kodi7rd.github.io/repository/other/DarkSubs_OpenSubtitles/darksubs_opensubtitles_api.json', timeout=DEFAULT_REQUEST_TIMEOUT).json()
        if isinstance(remote, list) and remote:
            seen = {k.get("OS_API_KEY_VALUE") for k in remote if isinstance(k, dict)}
            for k in DEFAULT_OS_API_KEYS:
                if k["OS_API_KEY_VALUE"] not in seen:
                    remote.append(k)
            return remote
    except Exception as e:
        log.warning('DEBUG | [OpenSubtitles] | remote key list unavailable, using baked keys: ' + repr(e))
    return list(DEFAULT_OS_API_KEYS)
...
def get_random_key():
    try:
        ...
        OS_API_KEYS = None
    if not OS_API_KEYS:                       # stale bad cache / cache miss
        OS_API_KEYS = list(DEFAULT_OS_API_KEYS)
