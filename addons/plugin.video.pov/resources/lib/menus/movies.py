...
class Movies:
	...
	def build_movie_content(self, position, tag):
		try:
			...
			########### KODIRDIL - per-item rating props (skin flags; cached OMDb) ###########
			try:
				from modules.kodirdil_ratings import rating_props
				props.update(rating_props(meta_get('rating'), imdb_id))
			except: pass
			##################################################################################
