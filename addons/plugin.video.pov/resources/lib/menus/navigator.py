...
class Navigator:
	...
	def settings(self):
		pov_str, manager_str, changelog_str, short_str = ls(32000), ls(32513), ls(32508), ls(32514)
	...
	def genres(self):
		...
		for genre, value in sorted(genre_list.items()):
			...
			########### KODIRDIL - per-genre icons (upstream hardcodes genres.png) ###########
			self._add_item({'mode': mode, 'action': action, 'genre_id': value[0], 'name': genre}, value[1] if len(value) > 1 and value[1] else 'genres.png', list_name=list_name)
			###################################################################################
		self._end_directory()
