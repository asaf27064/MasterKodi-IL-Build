...
def create_search_hebrew_subtitles_thread(media_type, title, season, episode, year, tmdb_id, imdb_id):
    ...
    if media_type == 'movie':
        ...
        kodi_utils.logger("POV-HEBSUBS", f"START search_hebrew_subtitles_thread - Movie: {title} | Year: {year} | TMDb ID: {tmdb_id}")
    else:
        hebrew_subtitles_search_arguments = ('tv', title, season, episode, year, tmdb_id, imdb_id)
        kodi_utils.logger("POV-HEBSUBS", f"START search_hebrew_subtitles_thread - TV: {title} S{season}E{episode} | Year: {year} | TMDb ID: {tmdb_id}")
    ...
    try:
        ...
        kodi_utils.logger("POV-HEBSUBS", f"ERROR creating search_hebrew_subtitles_thread: {str(e)}")
        return None
    ...
    try:
        ...
        while search_hebrew_subtitles_thread is not None and search_hebrew_subtitles_thread.is_alive():
            ...
            if count > 100:  # 10 seconds timeout
                kodi_utils.logger("POV-HEBSUBS", "FORCE STOP search_hebrew_subtitles_thread - exceeded 10 seconds")
                break
                
        elapsed_time = time.time() - search_hebrew_subtitles_thread_start_time
        kodi_utils.logger("POV-HEBSUBS", f"END search_hebrew_subtitles_thread - Total run time: {elapsed_time:.2f} seconds")
        
    except Exception as e:
        kodi_utils.logger("POV-HEBSUBS", f"ERROR managing search_hebrew_subtitles_thread: {str(e)}")
