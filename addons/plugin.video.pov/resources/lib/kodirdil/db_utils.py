...
def _profile():
    ...
    if _addon_profile is None:
        _addon_profile = kodi_utils.translate_path('special://profile/addon_data/plugin.video.pov/')
    ...
    with _conn_lock:
        if _hebrew_subs_conn is None:
            try:
                ...
                kodi_utils.logger("POV-HEBSUBS", "Error connecting to hebrew_subtitles_db: %s" % str(e))
                return None
    ...
    with _conn_lock:
        if _media_meta_conn is None:
            try:
                ...
                kodi_utils.logger("POV-HEBSUBS", "Error connecting to media_metadata_db: %s" % str(e))
                return None
...
def _ensure_tables():
    ...
    with _conn_lock:
        ...
        try:
            ...
            kodi_utils.logger("POV-HEBSUBS", "Error ensuring tables: %s" % str(e))
...
def write_unique_subtitles_to_hebrew_subtitles_db(unique_subtitles_list, website_subtitles_dict):
    ...
    try:
        ...
        kodi_utils.logger("POV-HEBSUBS", "Written %d subtitles to database" % len(unique_subtitles_list))
    except Exception as e:
        kodi_utils.logger("POV-HEBSUBS", "Error in writing subtitles to hebrew_subtitles_db: %s" % str(e))
...
def clear_hebrew_subtitles_db_cache():
    ...
    try:
        ...
        kodi_utils.logger("POV-HEBSUBS", "Error Clearing hebrew_subtitles_db cache: %s" % str(e))
...
def get_total_subtitles_found_list_from_hebrew_subtitles_db():
    ...
    try:
        ...
        kodi_utils.logger("POV-HEBSUBS", "Error while reading hebrew_subtitles_db: %s" % str(e))
        return []
...
def write_current_media_metadata_to_media_metadata_db(media_type, title, season, episode, year, tmdb_id):
    ...
    try:
        ...
        kodi_utils.logger("POV-HEBSUBS", "EMBEDDED | Error in writing to media_metadata_db: %s" % str(e))
...
def clear_media_metadata_db_cache():
    ...
    try:
        ...
        kodi_utils.logger("POV-HEBSUBS", "EMBEDDED | Error Clearing media_metadata_db cache: %s" % str(e))
...
def get_media_type_from_media_metadata_db():
    ...
    try:
        ...
        kodi_utils.logger("POV-HEBSUBS", "EMBEDDED | Error while reading media_metadata_db: %s" % str(e))
        return None
