				...
				try:
					...
					if enable_hebrew_subtitles and (total_subtitles_found_list or hebrew_embedded_taglines):
						try:
							...
							external_matched, embedded_matched, subtitle_matches_text, quality_counts, sync_badge = \
					...
					########### MASTERKODI - Synced pill badge (replaces the old orange size_label text) ###########
					# sources_results.xml draws a colored pill (top-right of the row) from these
					# properties. The old inline orange text on size_label is gone -- the badge
					# carries the same information and is scannable at a glance.
					size_label = get('size_label', 'N/A')
					has_hebrew_subs = 'false'
					heb_sync, heb_sync_color, heb_sync_label = '', '', ''
					if enable_hebrew_subtitles and sync_badge[0]:
						has_hebrew_subs = 'true'
						heb_sync = sync_badge[0]
						if heb_sync == 'embedded':
							heb_sync_color, heb_sync_label = 'FF2596BE', 'מוטמע ✓'
						elif heb_sync == 'full':
							heb_sync_color, heb_sync_label = 'FF1D9E75', 'מסונכרן %s%% ✓' % sync_badge[1]
						else:
							heb_sync_color, heb_sync_label = 'FFC9821E', 'חלקי %s%% ~' % sync_badge[1]
					##################################################################################################
					...
					set_properties({'name': name.upper(), 'source_site': source_site, 'provider_icon': provider_icon, 'quality_icon': quality_icon, 'count': '%02d.' % count,
							...
							'heb_sync': heb_sync, 'heb_sync_color': heb_sync_color, 'heb_sync_label': heb_sync_label})
					yield listitem
				except: pass
		try:
